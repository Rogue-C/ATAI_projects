...
DataPath = "DataFile"
FilePath = "DataFile/OutData/LSTM_CN_OUTDATA"
w2vmodel_path = f"{FilePath}/w2v.model"
finalmodel_path = f"{FilePath}/LSTM_CN.pth"
# 设置警告忽略，将一些无用的风险给忽略掉（仅仅是满足强迫症）
warnings.filterwarnings("ignore")
# 数据分析图像存放路径
GraphPath = "DataFile/OutData/LSTM_CN_OUTDATA/graph"
...
def read_training_data():
    ...
    fileters = ['!', '"', '#', '$', '%', '&', '\(', '\)', '\*', '\+', ',', '-', '\.', '/', ':', ';', '<', '=', '>',
                ...
                '\【', '\】', '\（', '\、', '。', '\）', '\，', '[\s0-9a-zA-Z]', '～', '！', '：', '《', '》', '×', '？', '＋', '＝',
                '·', '°', "'", '‘', '’', '；', '…', '﻿']
    with open(f"{DataPath}\\simplifyweibo_4_moods.csv", "r", encoding='UTF-8') as f:
        data = csv.reader(f, skipinitialspace=True)
        header = next(csv.reader(f, skipinitialspace=True))
        x = [row[1:] for row in data]
    x = list(np.array(x).ravel())
    x = [re.sub("|".join(fileters), " ", s) for s in x]  # 去除掉一些标点符号
    ...
    for tmp in x:
        words = []
        for tmpstr in tmp:
            words.append(list(word for word in jieba.cut(tmpstr)))
        tmp_words = sum(words, [])
        final_x.append(tmp_words)
    f.close()
    ...
    with open(f"{DataPath}\\simplifyweibo_4_moods.csv", "r", encoding='UTF-8') as f:
        data = csv.reader(f, skipinitialspace=True)
        header = next(csv.reader(f, skipinitialspace=True))
        x_label = [row[:1] for row in data]
    x_label = list(np.array(x_label).ravel())
    f.close()
    return final_x, x_label
...
"""
def test():
    model = torch.load(f'{FilePath}/LSTM_CN.pth')
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print("模型已加载！")
    print(next(model.parameters()).device)
    print("############请输入你要判断的文本############")
    string = input()
    train_x = [list(word for word in jieba.cut(string, cut_all=False))]
    print(train_x)
    preprocess = Preprocess(train_x, 33, w2v_path=w2vmodel_path)
    embedding = preprocess.make_embedding(load=True)
    train_x = preprocess.sentence_word2index()
    test_dataset = myDataset(x=train_x, y=None)
    test_loader = DataLoader(test_dataset, batch_size=64, shuffle=False, num_workers=0)
    model = model.to(device)
    outputs = testing(batch_size=64, test_loader=test_loader, model=model, device=device)
    print(outputs)
...
def testing(batch_size, test_loader, model, device):
    ...
    with torch.no_grad():
        for i, inputs in enumerate(test_loader):
            ...
            outputs[outputs >= 0.5] = 1
            outputs[outputs < 0.5] = 0
            ret_output.append(outputs.int().tolist())
        return ret_output
